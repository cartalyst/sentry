<?php namespace Cartalyst\Sentry\Users\Phalcon;
/**
...
class User extends Model implements UserInterface {
    ...
    public function hasPermission($permissions, $all = true)
    {
        ...
        foreach ($permissions as $permission)
        {
            ...
            if ((strlen($permission) > 1) and Text::endsWith($permission, '*'))
            {
                $matched = false;
                ...
                foreach ($mergedPermissions as $mergedPermission => $value)
                {
                    ...
                    if ($checkPermission != $mergedPermission and Text::startsWith($mergedPermission, $checkPermission) and $value == 1)
                    {
                        $matched = true;
                        break;
                    }
            ...
            elseif ((strlen($permission) > 1) and Text::startsWith($permission, '*'))
            {
                $matched = false;
                ...
                foreach ($mergedPermissions as $mergedPermission => $value)
                {
                    ...
                    if ($checkPermission != $mergedPermission and Text::endsWith($mergedPermission, $checkPermission) and $value == 1)
                    {
                        $matched = true;
                        break;
                    }
            ...
            else
            {
                ...
                foreach ($mergedPermissions as $mergedPermission => $value)
                {
                    ...
                    if ((strlen($mergedPermission) > 1) and Text::endsWith($mergedPermission, '*'))
                    {
                        $matched = false;
                        ...
                        // We will make sure that the merged permission does not
                        // exactly match our permission, but starts with it.
                        if ($checkMergedPermission != $permission and Text::startsWith($permission, $checkMergedPermission) and $value == 1)
                        {
                            $matched = true;
                            break;
                        }
